import { Component, Host, h } from '@stencil/core';

@Component({
  tag: 'azwc-tabs',
  // styleUrl: 'azwc-tabs.scss',
  shadow: false,
})
export class AzwcTabs {

  openCity(evt, cityName) {
    // Declare all variables
    var i, tabcontent, tablinks, tabcontents;

    // Get all elements with class="aztabcontents" and remove the class "active"
    tabcontents = document.getElementsByClassName("aztabcontent");
    for (i = 0; i < tabcontents.length; i++) {
      tabcontents[i].className = tabcontents[i].className.replace(" active", "");
    }

    // Get all elements with class="aztablinks" and remove the class "active"
    tablinks = document.getElementsByClassName("aztablinks");
    for (i = 0; i < tablinks.length; i++) {
      tablinks[i].className = tablinks[i].className.replace(" active", "");
    }
  
    // Show the current tab, and add an "active" class to the button that opened the tab
    tabcontent = document.getElementById(cityName);
    tabcontent.className += " active";
    evt.currentTarget.className += " active";
  }
  render() {
    return (
      <Host>
        <div class="aztab">
          <button class="aztablinks active" onClick={(event) => { this.openCity(event, 'London') }}>London</button>
          <button class="aztablinks" onClick={(event) => { this.openCity(event, 'Paris') }}>Paris</button>
          <button class="aztablinks" onClick={(event) => { this.openCity(event, 'Tokyo') }}>Tokyo</button>
        </div>
        <div id="London" class="aztabcontent active">
          <h3>London</h3>
          <p>London is the capital city of England.</p>
        </div>

        <div id="Paris" class="aztabcontent">
          <h3>Paris</h3>
          <p>Paris is the capital of France.</p>
        </div>

        <div id="Tokyo" class="aztabcontent">
          <h3>Tokyo</h3>
          <p>Tokyo is the capital of Japan.</p>
        </div>
      </Host>
    );
  }

}
